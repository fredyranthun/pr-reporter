...
case "$url" in
  ...
  https://github.com/fredyranthun/pr-reporter/releases/download/v0.1.0/checksums.txt|\
  https://github.com/fredyranthun/pr-reporter/releases/download/v0.1.0/pr-report-linux-amd64) ;;
  *) exit 1 ;;
esac
printf '%s\n' "$url" >> "$CURL_LOG"
...
grep -Fq 'pr-report v0.1.0' "$test_root/output"
...
write_checksums
run_installer > "$test_root/output"
grep -Fq '/releases/download/v0.1.0/checksums.txt' "$curl_log"
cmp "$fixture_dir/pr-report-linux-amd64" "$install_dir/pr-report"
