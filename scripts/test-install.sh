...
case "$url" in
  ...
  https://github.com/fredyranthun/pr-reporter/releases/download/v0.1.2/checksums.txt|\
  https://github.com/fredyranthun/pr-reporter/releases/download/v0.1.2/pr-report-linux-amd64) ;;
  *) exit 1 ;;
esac
printf '%s\n' "$url" >> "$CURL_LOG"
...
grep -Fq 'pr-report v0.1.2' "$test_root/output"
...
test "$("$install_dir/pr-report" --version)" = 'pr-report v0.1.1'
run_installer > "$test_root/output"
grep -Fq 'pr-report v0.1.2' "$test_root/output"
cmp "$fixture_dir/pr-report-linux-amd64" "$install_dir/pr-report"
...
write_checksums
run_installer > "$test_root/output"
grep -Fq '/releases/download/v0.1.2/checksums.txt' "$curl_log"
cmp "$fixture_dir/pr-report-linux-amd64" "$install_dir/pr-report"
